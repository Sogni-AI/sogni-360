...
function Step3({ step1, step2, onReturn, onContinue }: Props) {
  ...
  const doSignup = useCallback(async () => {
    // Only require turnstile token when key is configured (production)
    const hasTurnstile = !!getTurnstileKey();
    ...
    <FormPanel onSubmit={handleFormSubmit} disabled={isLoading} autoComplete="off">
      <FormContent subHeading="Review your credentials">
        <div className="login-modal-password-warning">
          <strong>Credentials cannot be reset or recovered.</strong>
      ...
      <FormFooter>
        ...
        <button type="submit" className="login-modal-button primary" disabled={(getTurnstileKey() && !turnstileToken) || isLoading}>
