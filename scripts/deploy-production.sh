...
# Comprehensive production deployment script for Sogni 360
# This script handles building, deploying, and starting both frontend and backend

set -e # Exit immediately if a command exits with a non-zero status

echo "🚀 Starting Sogni 360 Production Deployment"
echo "=================================================="

# Configuration
REMOTE_HOST="sogni-staging"
REMOTE_FRONTEND_PATH="/var/www/360.sogni.ai"
REMOTE_BACKEND_PATH="/var/www/360.sogni.ai-server"
...
rsync -ar --progress scripts/nginx/production.conf $REMOTE_HOST:/tmp/sogni-360-nginx.conf
if [ $? -ne 0 ]; then
...
ssh $REMOTE_HOST /bin/bash --noprofile --norc << 'EOF'
  ...
  cd /var/www/360.sogni.ai-server
  ...
  pm2 delete sogni-360-production 2>/dev/null || true
  PORT=3002 pm2 start index.js --name sogni-360-production
  pm2 save
  ...
  pm2 status sogni-360-production

  # Check that nginx is properly configured
  echo "🔍 Verifying nginx configuration..."
  sudo cp /tmp/sogni-360-nginx.conf /etc/nginx/conf.d/360.sogni.ai.conf
  sudo nginx -t
  if [ $? -eq 0 ]; then
...
echo "🔍 Checking backend health via branded domain (https://360-api.sogni.ai)..."
HEALTH_CHECK=$(ssh $REMOTE_HOST "curl -s -o /dev/null -w '%{http_code}' https://360-api.sogni.ai/health" || echo "failed")
if [ "$HEALTH_CHECK" = "200" ]; then
  ...
  echo "⚠️ Warning: The backend may not be running correctly. Please check logs with: ssh $REMOTE_HOST 'pm2 logs sogni-360-production'"
fi

echo "🔍 Checking frontend availability (https://360.sogni.ai)..."
FRONTEND_CHECK=$(curl -s -o /dev/null -w "%{http_code}" -I https://360.sogni.ai/ || echo "failed")
...
echo "Frontend: https://360.sogni.ai/"
echo "Backend API: https://360-api.sogni.ai/"
