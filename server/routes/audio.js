...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    // Accept MP3, M4A, WAV files
    const allowedMimes = ['audio/mpeg', 'audio/mp3', 'audio/mp4', 'audio/x-m4a', 'audio/m4a', 'audio/wav', 'audio/wave', 'audio/x-wav'];
    ...
    if (allowedMimes.includes(file.mimetype) || allowedExts.includes(ext)) {
      ...
      cb(new Error(`Unsupported audio format. Received: ${file.mimetype} (${ext}). Supported: MP3, M4A, WAV`), false);
    }
  }
});
...
// Transcode MP3/WAV to M4A endpoint
